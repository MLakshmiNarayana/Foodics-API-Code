    ...
    public partial class Form1 : Form
    {
        ...
        private void Load_orders()
        {
            ...
            try
            {
                ...
                dgv.Columns[1].DataPropertyName = "Focus Account";
        ...
        public void saveorders()
        {
            try
            {
                ...
                //qry = "select distinct ori.Order_RefNo,ori.Business_date,br.Name as BranchName,ori.Branch_hid,ori.Order_Final_price,ori.Tax_amount,(select[Focus Account] from vsm_FocusAcc_PaymentMthd where sno = 0)[SalesAccount],pm.Name as Payment_method_name,fpm.[Focus Account], op.Amount from Orders ori join Branches br on br.hid = ori.Branch_hid join Order_PaymentMethods op on op.Order_refNo = ori.Order_RefNo join PaymentMethods pm on pm.hid = op.payment_method_hid join vsm_FocusAcc_PaymentMthd fpm on fpm.[PaymentMethod] collate Arabic_CI_AS = pm.Name  collate Arabic_CI_AS where br.Name = '"+cmbbr.Text+"' and ori.Business_date = '"+date+"' order by ori.Order_RefNo asc";
                //DataSet sd = ClsSql.GetDs(qry);

                sMysql = "select DISTINCT Order_RefNo from [v_salesorder] WHERE Order_RefNo = '"+ refno + "'";
                ...
                for (int d = 0; d < ds.Tables[0].Rows.Count; d++)
                {
                    ...
                    mysql = "select *, case when Payment_method_name = 'Cash' then 1862 else 1864 end AccId from [v_salesorder] WHERE Order_RefNo = '" + sRefNo + "'";
                    ...
                    for (int i = 0; i < dst.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (icnt == 0)
                        {
                            ...
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "DocNo", Value = refno });
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "Date", Value = 132317450 });
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "Branch", Value = "1" });// "Jeddah Branch B01" });
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "Division", Value = "1" });// "Funky Monky" });
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "Placeof supply", Value = "1" });
                            lstHeaderData.Add(new Foodicsposting.TransactionService.IdNameTag { sName = "Jurisdiction", Value = "1" });
